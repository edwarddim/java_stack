...
class SinglyLinkedList {
   ...
   */
  hasLoop() { 
      // let slow, fast pointers point at head node
      let slow = this.head, 
          fast = this.head
      
      while (fast != null & fast.next != null){
          // fast pointer moves two steps every time
          fast = fast.next.next
          // slow pointer moves one step every time
          slow = slow.next
          // if they meet in one node, loop is found
          if (slow == fast){
              return true
          }
      }
      // after iteration, there is no loop, return false
      return false
  }
  /**

...
class SinglyLinkedList {
   ...
   */
   ...
   reverse() {
    ...
    var current = this.head; //keep track of original head
    var next = null; //instantiate a next
    while (current != null) { //while the head exists (we haven't moved it to the back)
        next = current.next; //set the next value to be the 2nd item in the list
        current.next = prev; //set the current head.next to null
        prev = current; //reassign previous to be this head
        current = next; //reassign "this head" to be the value next to the current head (moving it "in place")
    }
    this.head = prev; //finally reassign this.head which should be null to prev which is the reversed SLL
